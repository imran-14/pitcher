...
$query = "MATCH (user {username:'$user'}) RETURN user.name as name,user.profilepic as profilepic,user.coverpic as coverpic,user.no_of_pics_uploaded as number";
$result = $client->run($query);
$record = $result->getRecord();
$name = $record->value('name');
$profilepic = $record->value('profilepic');
$coverpic = $record->value('coverpic');
$no_of_pics = $record->value('number');

$result1 = $client->run("MATCH (user:USERS {username:'$user'})-[r:HAS_POSTED]->(pic:IMAGE) RETURN pic,pic.filename as filename,pic.posted_time as posted_time  ORDER BY pic.posted_time DESC");


		/*foreach ($result1->records() as $record1) {
   		 	//print_r($record1->get('pic'); // nodes returned are automatically hydrated to Node objects

    			echo $record1->value('filename') ;
    			echo date('d/m/y H:i:s',$record1->value('posted_time'));
		}*/
		...
		<div class="container">
			...
			<div class="content">
				<div class="cover-pic-box">
					<img src="users/<?php echo $user."/".$coverpic?>" class="cover-pic">
					<img src="users/<?php echo $user."/".$profilepic?>" class="profile-pic">
				</div>
				<?php foreach ($result1->records() as $record1) {?>
				<div class="postImageBorder">
					<div class="imagePost">
						<div class="userPosted"><a href=""><?php echo $name." Posted at : ".date('d/m/y H:i:s',$record1->value('posted_time'));?></a></div>
						<table class="postContent">
							<tr>
								<td style='width: 60%;'>
									<img src="users/<?php echo $user?>/<?php echo $record1->value('filename')?>" class="postImage">
									<ul class="postAlign">
